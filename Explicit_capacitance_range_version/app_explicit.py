...
try:
    from analysis_explicit import ImpedanceAnalyzer
except ImportError:
...
def plot_3d_impedance_vs_capacitance(all_data: pd.DataFrame, steak_size: float, y_column: str, y_label: str) -> Tuple[
    ...
    fig.update_layout(
        scene=dict(xaxis=dict(title="Capacitance (pF)"),
    ...
    df_results = min_mag_data[["Relative Variable", "capacitance", y_column]].rename(columns={
        "capacitance": "Capacitance (pF)", y_column: f"{y_label} at Resonance (Ω)"
    })
    return (fig, df_results)
...
with st.sidebar:
    ...
    if plot_type_str == '|Z| (Parallel Model)' and coil_inductance <= 0:
        ...
        st.stop()
    if not cap_values_list:
        st.error("Please enter at least one capacitance value.")
        st.stop()
