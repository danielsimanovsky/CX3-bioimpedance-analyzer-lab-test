...
class ImpedanceAnalyzer:
    ...
    def run_spec_to_csv_conversion(self, cap_start: float, cap_step: float) -> str:
        ...
        new_names = []
        # Custom logic: first 10 are 0.01 steps from cap_start
        for i in range(10):
            cap_value = cap_start + (0.01 * i)
            new_names.append(f"{cap_value:.2f}_nf.csv")

        # Remaining files start from 0.1 and use cap_step
        remaining_count = len(spec_files) - 10
        if remaining_count > 0:
            for i in range(remaining_count):
                cap_value = 0.1 + (i * cap_step)
                new_names.append(f"{cap_value:.2f}_nf.csv")
                # --- END UPDATED LOGIC ---
    ...
    def plot_2d_graphs(self, plot_type: str) -> Tuple[plt.Figure, plt.Figure, plt.Figure, pd.DataFrame, pd.DataFrame]:
        ...
        for file in csv_files:
            ...
            try:
                ...
                if len(mags) > 0:
                    # --- PARALLEL CALCULATION (Safe even if not used) ---
                    L_henry = self.coil_inductance * 1e-9
                    C_farad = cap_val * 1e-9
                    epsilon = 1e-12
        ...
        ax3.set_xlabel("Capacitance (nF)")
        ax3.set_ylabel(f"Frequency at Max {y_label} (MHz)")  # Changed Label
